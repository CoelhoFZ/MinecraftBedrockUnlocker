...
fn set_console_size() {
    unsafe {
        ...
        if let Ok(handle) = GetStdHandle(STD_OUTPUT_HANDLE) {
            ...
            // SECOND: Set exact buffer size (84x30)
            let buffer_size = COORD { X: 84, Y: 30 };
...
fn print_help() {
    ...
    println!("  instalar-mod          - Instalar DLL modificada");
    println!("  restaurar-original    - Restaurar DLL original");
    println!("  abrir-minecraft       - Abrir o Minecraft");
    println!("  abrir-store           - Abrir Microsoft Store do Minecraft");
    println!("  status                - Verificar status atual");
    println!("  ajuda                 - Mostrar esta mensagem");
    println!("  sair                  - Sair do programa");
}

fn is_elevated() -> bool {
    // For now, assume elevated
    // TODO: Implement real check or use external library
    true
}
