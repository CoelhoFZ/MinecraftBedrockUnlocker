<#
.SYNOPSIS
  Minecraft Bedrock Unlocker v3.2.0 - Unified bootstrap (PowerShell).
...
function T {
    ...
    $table = @{
        en = @{
            ...
            'press_enter_pt' = 'Press ENTER to exit'
        }
        zh = @{
            'banner' = 'Minecraft 基岩版解锁器 - 引导 v3.2.0'
            'detecting_os' = '正在检测平台...'
            'os_unsupported' = '此引导程序需要 Windows。'
            ...
            'sha_skip' = 'SHA256SUMS.txt 不可用；跳过哈希检查。'
            'launching_admin' = '正以管理员身份启动 {0}（UAC 提示）...'
            'fallback_header' = 'EXE 启动失败。回退到从 raw/main 拉取 unlocker.ps1...'
        ...
        hi = @{
            ...
            'os_unsupported' = 'इस बूटस्ट्रैप के लिए Windows आवश्यक है।'
        ...
        es = @{
            ...
            'os_unsupported' = 'Este bootstrap requiere Windows.'
            'ps_unsupported' = 'Se requiere PowerShell 5.1 o mas reciente (usted tiene {0}).'
            'downloading_exe' = 'Descargando {0} desde {1}...'
            'verify_pe' = 'Verificando firma del EXE (encabezado PE32+ MZ)...'
            ...
            'sha_skip' = 'SHA256SUMS.txt no disponible; omitiendo verificacion de hash.'
            'launching_admin' = 'Iniciando {0} como Administrador (aviso UAC)...'
            'fallback_header' = 'El EXE fallo al iniciar. Volviendo a unlocker.ps1 desde raw/main...'
            'downloading_ps1' = 'Descargando unlocker.ps1 desde {0}...'
            'verify_ps1' = 'Verificando unlocker.ps1 (tamano + tipo de contenido)...'
            'running_ps1' = 'Ejecutando unlocker.ps1 con PowerShell...'
            'all_failed' = 'Todos los intentos fallaron. Posibles causas:'
            'cause_internet' = '  1. Sin conexion a internet o GitHub no accesible'
            'cause_av' = '  2. El antivirus bloqueo la descarga'
            'cause_release' = '  3. Los assets del release de GitHub no estan disponibles temporalmente'
            'cause_local' = '  4. PowerShell local no puede elevarse a Administrador'
            'suggestion' = 'Sugerencias:'
            'suggest_disable_av' = '  - Desactive temporalmente la proteccion antivirus en tiempo real'
            'suggest_wait' = '  - Espera unos minutos y reintenta (GitHub puede estar limitando la tasa)'
            'suggest_release' = '  - Abre https://github.com/{0}/{1}/releases y descarga el EXE manualmente'
            'enter_to_exit' = 'Presione ENTER para salir'
            'press_enter_pt' = 'Presione ENTER para salir'
        }
        fr = @{
            'banner' = 'Minecraft Bedrock Unlocker - bootstrap v3.2.0'
            'detecting_os' = 'Detection de la plateforme...'
            'os_unsupported' = 'Ce bootstrap necessite Windows.'
            'ps_unsupported' = 'PowerShell 5.1 ou plus recent est requis (vous avez {0}).'
            'downloading_exe' = 'Telechargement de {0} depuis {1}...'
            'verify_pe' = 'Verification de la signature EXE (en-tete PE32+ MZ)...'
            'verify_sha' = 'Verification de SHA256 par rapport a SHA256SUMS.txt...'
            'sha_match' = 'SHA256 OK.'
            'sha_mismatch' = 'SHA256 ne correspond pas. Attendu {0}, obtenu {1}.'
            'sha_skip' = 'SHA256SUMS.txt indisponible; verification de l empreinte ignoree.'
            'launching_admin' = 'Lancement de {0} en tant qu Administrateur (invite UAC)...'
            'fallback_header' = 'Echec du lancement de l EXE. Retour a unlocker.ps1 depuis raw/main...'
            'downloading_ps1' = 'Telechargement de unlocker.ps1 depuis {0}...'
            'verify_ps1' = 'Verification de unlocker.ps1 (taille + type de contenu)...'
            'running_ps1' = 'Execution de unlocker.ps1 avec PowerShell...'
            'all_failed' = 'Toutes les tentatives ont echoue. Causes possibles:'
            'cause_internet' = '  1. Pas de connexion Internet ou GitHub injoignable'
            'cause_av' = '  2. L antivirus a bloque le telechargement'
            'cause_release' = '  3. Les ressources du release GitHub sont temporairement indisponibles'
            'cause_local' = '  4. PowerShell local ne peut pas s elever en Administrateur'
            'suggestion' = 'Suggestions:'
            'suggest_disable_av' = '  - Desactivez temporairement la protection antivirus en temps reel'
            'suggest_wait' = '  - Attendez quelques minutes et reessayez (GitHub limite peut-etre le debit)'
            'suggest_release' = '  - Ouvrez https://github.com/{0}/{1}/releases et telechargez l EXE manuellement'
            'enter_to_exit' = 'Appuyez sur ENTREE pour quitter'
            'press_enter_pt' = 'Appuyez sur ENTREE pour quitter'
        }
        ar = @{
            'banner' = 'Minecraft Bedrock Unlocker - bootstrap v3.2.0'
            'detecting_os' = 'جارٍ اكتشاف النظام الأساسي...'
            'os_unsupported' = 'هذا المُحمِّل يتطلب Windows.'
            ...
            'sha_skip' = 'SHA256SUMS.txt غير متوفر؛ تخطي فحص التجزئة.'
            ...
            'cause_release' = '  3. أصول إصدار GitHub غير متاحة مؤقتًا'
            'cause_local' = '  4. لا يستطيع PowerShell المحلي الارتفاع إلى صلاحية المسؤول'
            'suggestion' = 'اقتراحات:'
            'suggest_disable_av' = '  - قم بتعطيل الحماية في الوقت الفعلي من برنامج مكافحة الفيروسات مؤقتًا'
            'suggest_wait' = '  - انتظر بضع دقائق وحاول مرة أخرى (قد يكون GitHub يحد من المعدل)'
            'suggest_release' = '  - افتح https://github.com/{0}/{1}/releases وقم بتنزيل EXE يدويًا'
        ...
        ru = @{
            ...
            'os_unsupported' = 'Этот загрузчик требует Windows.'
            ...
            'sha_skip' = 'SHA256SUMS.txt недоступен; пропуск проверки хеша.'
            ...
            'cause_local' = '  4. Локальный PowerShell не может повысить права до администратора'
            'suggestion' = 'Предложения:'
            'suggest_disable_av' = '  - Временно отключите антивирусную защиту в реальном времени'
            'suggest_wait' = '  - Подождите несколько минут и повторите (GitHub может ограничивать скорость)'
