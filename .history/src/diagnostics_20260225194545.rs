...
impl Diagnostics {
    ...
    /// Check if Minecraft was installed from Xbox App (GDK) vs Microsoft Store (UWP)
    fn check_installation_type(&self) -> DiagnosticResult {
        let path_str = self.minecraft_path.display().to_string();
        
        let is_xbox_app = path_str.contains("XboxGames");
        let is_windows_apps = path_str.contains("WindowsApps");
        
        if is_xbox_app {
            DiagnosticResult {
                name: diag_install_type().to_string(),
                passed: true,
                message: format!("{} ({})", diag_xbox_gdk(), diag_compatible()),
                suggestion: None,
            }
            ...
            DiagnosticResult {
                name: diag_install_type().to_string(),
                passed: false,
                message: format!("{} ({})", diag_ms_store(), diag_not_compatible()),
                suggestion: Some(diag_reinstall_xbox().to_string()),
            }
        } else {
            DiagnosticResult {
                name: diag_install_type().to_string(),
                passed: false,
                message: diag_unknown_install().to_string(),
                suggestion: Some(diag_reinstall_xbox().to_string()),
            }
        }
    }

    /// Check if bypass files are present and not deleted by antivirus
    fn check_bypass_files(&self) -> DiagnosticResult {
        ...
        if found == 0 {
            DiagnosticResult {
                name: diag_bypass_files().to_string(),
                passed: true, // Not installed is OK, just informational
                message: diag_not_installed_yet().to_string(),
                suggestion: Some(diag_use_option_1().to_string()),
            }
        } else if missing.is_empty() {
            DiagnosticResult {
                name: diag_bypass_files().to_string(),
                passed: true,
                message: diag_all_present().to_string(),
                suggestion: None,
            }
        } else {
            // Partial - antivirus likely deleted some files
            let missing_str = missing.join(", ");
            DiagnosticResult {
                name: diag_bypass_files().to_string(),
                passed: false,
                message: format!("{}: {}", diag_missing_files(), missing_str),
                suggestion: Some(diag_av_deleted().to_string()),
            }
        }
    }
...
fn diag_install_type() -> &'static str {
    match get_language() {
        Language::English => "Installation Type",
        Language::PortugueseBR | Language::PortuguesePT => "Tipo de Instalação",
        Language::Spanish => "Tipo de Instalación",
        Language::French => "Type d'Installation",
        Language::German => "Installationstyp",
        Language::ChineseSimplified => "安装类型",
        Language::Russian => "Тип установки",
    }
}

fn diag_xbox_gdk() -> &'static str {
    match get_language() {
        Language::English => "Xbox App (GDK)",
        Language::PortugueseBR | Language::PortuguesePT => "Xbox App (GDK)",
        _ => "Xbox App (GDK)",
    }
}

fn diag_ms_store() -> &'static str {
    match get_language() {
        Language::English => "Microsoft Store (UWP)",
        Language::PortugueseBR | Language::PortuguesePT => "Microsoft Store (UWP)",
        _ => "Microsoft Store (UWP)",
    }
}

fn diag_compatible() -> &'static str {
    match get_language() {
        Language::English => "Compatible",
        Language::PortugueseBR | Language::PortuguesePT => "Compatível",
        Language::Spanish => "Compatible",
        Language::French => "Compatible",
        Language::German => "Kompatibel",
        Language::ChineseSimplified => "兼容",
        Language::Russian => "Совместимо",
    }
}

fn diag_not_compatible() -> &'static str {
    match get_language() {
        Language::English => "NOT COMPATIBLE!",
        Language::PortugueseBR | Language::PortuguesePT => "NÃO COMPATÍVEL!",
        Language::Spanish => "¡NO COMPATIBLE!",
        Language::French => "NON COMPATIBLE!",
        Language::German => "NICHT KOMPATIBEL!",
        Language::ChineseSimplified => "不兼容！",
        Language::Russian => "НЕ СОВМЕСТИМО!",
    }
}

fn diag_reinstall_xbox() -> &'static str {
    match get_language() {
        Language::English => "Uninstall and reinstall Minecraft from Xbox App (NOT Microsoft Store!)",
        Language::PortugueseBR | Language::PortuguesePT => "Desinstale e reinstale o Minecraft pelo Xbox App (NÃO Microsoft Store!)",
        Language::Spanish => "Desinstale y reinstale Minecraft desde Xbox App (¡NO Microsoft Store!)",
        Language::French => "Désinstallez et réinstallez Minecraft depuis Xbox App (PAS Microsoft Store!)",
        Language::German => "Deinstallieren und neu installieren über Xbox App (NICHT Microsoft Store!)",
        Language::ChineseSimplified => "从Xbox App卸载并重新安装Minecraft（不是Microsoft Store！）",
        Language::Russian => "Удалите и переустановите Minecraft из Xbox App (НЕ Microsoft Store!)",
    }
}

fn diag_unknown_install() -> &'static str {
    match get_language() {
        Language::English => "Unknown installation location",
        Language::PortugueseBR | Language::PortuguesePT => "Local de instalação desconhecido",
        Language::Spanish => "Ubicación de instalación desconocida",
        Language::French => "Emplacement d'installation inconnu",
        Language::German => "Unbekannter Installationsort",
        Language::ChineseSimplified => "未知的安装位置",
        Language::Russian => "Неизвестное место установки",
    }
}

fn diag_bypass_files() -> &'static str {
    match get_language() {
        Language::English => "Bypass Files",
        Language::PortugueseBR | Language::PortuguesePT => "Arquivos do Bypass",
        Language::Spanish => "Archivos del Bypass",
        Language::French => "Fichiers du Bypass",
        Language::German => "Bypass-Dateien",
        Language::ChineseSimplified => "绕过文件",
        Language::Russian => "Файлы обхода",
    }
}

fn diag_not_installed_yet() -> &'static str {
    match get_language() {
        Language::English => "Not installed yet",
        Language::PortugueseBR | Language::PortuguesePT => "Ainda não instalado",
        Language::Spanish => "Aún no instalado",
        Language::French => "Pas encore installé",
        Language::German => "Noch nicht installiert",
        Language::ChineseSimplified => "尚未安装",
        Language::Russian => "Ещё не установлено",
    }
}

fn diag_use_option_1() -> &'static str {
    match get_language() {
        Language::English => "Use option [1] to install the bypass",
        Language::PortugueseBR | Language::PortuguesePT => "Use a opção [1] para instalar o bypass",
        Language::Spanish => "Use la opción [1] para instalar el bypass",
        Language::French => "Utilisez l'option [1] pour installer le bypass",
        Language::German => "Verwenden Sie Option [1] um den Bypass zu installieren",
        Language::ChineseSimplified => "使用选项[1]安装绕过",
        Language::Russian => "Используйте опцию [1] для установки обхода",
    }
}

fn diag_all_present() -> &'static str {
    match get_language() {
        Language::English => "All files present",
        Language::PortugueseBR | Language::PortuguesePT => "Todos os arquivos presentes",
        Language::Spanish => "Todos los archivos presentes",
        Language::French => "Tous les fichiers présents",
        Language::German => "Alle Dateien vorhanden",
        Language::ChineseSimplified => "所有文件存在",
        Language::Russian => "Все файлы на месте",
    }
}

fn diag_missing_files() -> &'static str {
    match get_language() {
        Language::English => "Missing",
        Language::PortugueseBR | Language::PortuguesePT => "Faltando",
        Language::Spanish => "Faltando",
        Language::French => "Manquant",
        Language::German => "Fehlend",
        Language::ChineseSimplified => "缺失",
        Language::Russian => "Отсутствует",
    }
}

fn diag_av_deleted() -> &'static str {
    match get_language() {
        Language::English => "Antivirus likely deleted files! Add exclusion and reinstall.",
        Language::PortugueseBR | Language::PortuguesePT => "Antivírus provavelmente deletou arquivos! Adicione exclusão e reinstale.",
        Language::Spanish => "¡El antivirus probablemente eliminó archivos! Agregue exclusión y reinstale.",
        Language::French => "L'antivirus a probablement supprimé des fichiers! Ajoutez une exclusion et réinstallez.",
        Language::German => "Antivirus hat wahrscheinlich Dateien gelöscht! Fügen Sie Ausnahme hinzu und installieren Sie neu.",
        Language::ChineseSimplified => "杀毒软件可能删除了文件！添加排除并重新安装。",
        Language::Russian => "Антивирус вероятно удалил файлы! Добавьте исключение и переустановите.",
    }
}
